import { type CodePoint, type Step } from "./types.ts";

/** Per-character aggregate for a single lesson result. */
export type Sample = {
  readonly codePoint: CodePoint;
  readonly hitCount: number;
  readonly missCount: number;
  /** Mean time-to-type (ms) over correct presses of this char; 0 if none. */
  readonly timeToType: number;
};

/**
 * Aggregates the keystrokes of one lesson by character. Faithful port of
 * keybr's Histogram: typo presses contribute to missCount but not to the mean
 * time, and implausibly fast/slow samples are dropped.
 */
export class Histogram implements Iterable<Sample> {
  ...
  /** Number of distinct characters typed. */
  ...
  static from(steps: readonly Step[]): Histogram {
    const samples = new Map<
      CodePoint,
      { hitCount: number; missCount: number; time: number; count: number }
    >();
    for (const { codePoint, timeToType, typo } of steps) {
      ...
      if (sample == null) {
        samples.set(
          codePoint,
          (sample = { hitCount: 0, missCount: 0, time: 0, count: 0 }),
        );
      }
      sample.hitCount += 1;
      if (typo) {
